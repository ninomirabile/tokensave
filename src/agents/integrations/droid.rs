...
/// Regex matcher (Factory treats the value as a regex) covering Droid's three
...
/// whose name merely contains one as a substring; anchored `Task` matching was
/// verified live.
///
...
/// them would strand legitimate reads. Unmatched tools never reach our hook
/// subprocess, the safest "fail open".
const DROID_HOOK_MATCHER: &str = "^(Execute|Grep|Task)$";
...
impl AgentIntegration for DroidIntegration {
    ...
    fn install(&self, ctx: &InstallContext) -> Result<()> {
        ...
        crate::agent_note!("  2. Start a new droid session — tokensave tools are now available");
        Ok(())
    }
    ...
    fn uninstall(&self, ctx: &InstallContext) -> Result<()> {
        ...
        crate::agent_note!();
        crate::agent_note!("Uninstall complete. Tokensave has been removed from Factory Droid.");
        crate::agent_note!("Start a new droid session for changes to take effect.");
        Ok(())
    }
...
fn install_prompt_rules(agents_md: &Path) -> Result<()> {
    let body = rules_for_agent("droid")?;
    write_rules_block(agents_md, "droid", &body).map(|_| ())
}
...
/// current `^(Execute|Grep|Task)$` matcher: an entry already carrying that
...
fn install_hook(settings_path: &Path, tokensave_bin: &str) -> Result<()> {
    ...
    if already_current {
        crate::agent_note!("  {DROID_PRE_TOOL_EVENT} hook already present, skipping");
        return Ok(());
    }
...
fn doctor_check_prompt(dc: &mut DoctorCounters, home: &Path) {
    ...
    check_shared_rules_block(dc, &agents_md, "droid");
}
