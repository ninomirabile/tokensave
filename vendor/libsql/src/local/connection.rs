...
impl Connection {
    ...
    ///
    /// TOKENSAVE PATCH (tokensave#367, upstream libsql#2251 / PR libsql#2261):
    /// `disconnect` was not idempotent, so every local connection was closed
    /// twice. `LibsqlConnection::drop` (local/impls.rs) calls `disconnect()`,
    /// then the inner `Connection` field's own `Drop` calls it again; the
    /// `Arc::get_mut(drop_ref)` guard only proves unique ownership, which is
    /// true on both passes, and `raw` was left dangling after the first
    /// `sqlite3_close_v2`. The second call reads the freed `sqlite3` object —
    /// benign on glibc/macOS (the page stays mapped, so it returns
    /// `SQLITE_MISUSE`), a `STATUS_ACCESS_VIOLATION` on Windows once the block
    /// has been recycled. Nulling `raw` after the close makes the second pass a
    /// no-op. Remove this patch once the upstream fix ships.
